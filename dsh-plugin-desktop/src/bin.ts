/** Headless-safe npm launcher for the AdvanceMind Electron executable. */
...
Launch AdvanceMind with the selected Web-capable profile.
...
export function defaultDesktopUserDataDirectory(
  ...
  if (platform === 'win32') {
    ...
    if (appData === undefined || appData.length === 0) {
      throw new Error('APPDATA is unavailable; cannot locate AdvanceMind diagnostics')
    }
...
async function launchElectron(): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(
      ...
      + 'Or use the packaged AdvanceMind application.\n',
    )
    return 1
  }
