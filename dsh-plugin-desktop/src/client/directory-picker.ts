...
export async function requestDesktopDirectory(
  ...
  if (!response.ok) throw new Error('AdvanceMind could not open the system folder picker')
  const value: unknown = await response.json()
  if (!isResponse(value)) throw new Error('AdvanceMind received an invalid response from the system folder picker')
  return value.path
}
...
export async function requestDesktopDirectoryValidation(
  ...
  if (!response.ok) throw new Error('AdvanceMind could not validate the selected workspace')
  const value: unknown = await response.json()
  if (!isValidationResponse(value)) throw new Error('AdvanceMind received an invalid workspace validation response')
