...
describe('desktop terminal environment', () => {
  ...
  it('generates private macOS shims and opens one quoted welcome command', () => {
    ...
    expect(welcome).toContain('AdvanceMind 2.0.0 terminal')
    ...
    expect(welcome).toContain('Restart AdvanceMind after plugin changes.')
  ...
  it('generates Windows batch shims and opens PowerShell through a visible-console broker', () => {
    ...
    expect(welcome).toContain('"AdvanceMind {0} terminal" -f $env:DSH_DESKTOP_PRODUCT_VERSION')
    ...
    expect(launcher).toContain('start "AdvanceMind" /D "!DSH_DESKTOP_PROFILE_DIRECTORY!"')
